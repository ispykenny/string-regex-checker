...
export const isValidStateAbbreviation = (stateAbbreviation: string): boolean =>
  /^[A-Z]{2}$/.test(stateAbbreviation);

// Function to check if a string is a valid IPv4 address
export const isValidIPv4Address = (ipv4Address: string): boolean =>
  /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/.test(
    ipv4Address
  );

// Function to check if a string is a valid IPv6 address
export const isValidIPv6Address = (ipv6Address: string): boolean =>
...
export const isValidHexColorCode = (hexColorCode: string): boolean =>
  /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(hexColorCode);

// Function to check if a string is a valid RGB color code (e.g. rgb(255,255,255))
export const isValidRgbColorCode = (rgbColorCode: string): boolean =>
...
export const isValidCssColorCode = (cssColorCode: string): boolean =>
